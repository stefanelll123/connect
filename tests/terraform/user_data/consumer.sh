#!/bin/bash
# =============================================================================
# Consumer user_data — EC2 bootstrap for the consumer sentinel node.
# Installs Docker, clones repo, writes secrets env, then delegates to
# tests/terraform/scripts/consumer-setup.sh.
#
...
echo "=== Connect Consumer Bootstrap — $(date -u) ==="
...
CONSUMER_PRIVATE_IP=$(curl -s -H "X-aws-ec2-metadata-token: $TOKEN" \
...
CONSUMER_PRIVATE_IP=$CONSUMER_PRIVATE_IP
...
export CONSUMER_PRIVATE_IP
bash /opt/connect/tests/terraform/scripts/consumer-setup.sh